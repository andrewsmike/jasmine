...
class ReloadStateMachine(StateMachine[ReloadState, ReloadEvent]):
    """
    ...
    nice_name: str = "Reload"

    # Hack so we can typecheck state values, but still have them available here.
    states: set[ReloadState] = set(ReloadState.__args__)  # type: ignore
    ...
    events: set[ReloadEvent] = set(ReloadEvent.__args__)  # type: ignore
...
@orm_registry.mapped
class ReloadMaterialization(Materialization):
    acceptable_view_types = {"query"}
    state_machine_type: Type[StateMachine] = ReloadStateMachine
    materialization_name: str = "reload"

    config_types: dict[str, Type[Any]] = {}
    config_defaults: dict[str, Any | None] = {}

    __mapper_args__ = {
        "polymorphic_identity": "reload",
    }
