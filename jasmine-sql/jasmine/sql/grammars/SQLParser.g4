...
sqlProgram:
    EOF
    | statement (SEMICOLON_SYMBOL statement)* SEMICOLON_SYMBOL? EOF
;


statement:
    simpleStatement | beginWork
;
...
simpleStatement:
    ...
    | {self.serverVersion >= 80000}? importStatement
    ...
    // Data Directory
    | {self.serverVersion >= 80000}? cloneStatement
    ...
    | showStatement
    | {self.serverVersion >= 80000}? resourceGroupManagement
    | otherAdministrativeStatement

    // MySQL utilitity statements
    | utilityStatement
    | {self.serverVersion >= 50604}? getDiagnostics
    | signalStatement
    | resignalStatement
;
...
alterStatement:
    ALTER_SYMBOL (
        ...
        | alterEvent
        | alterTablespace
        | {self.serverVersion >= 80014}? alterUndoTablespace
        | alterLogfileGroup
        | alterServer
        // ALTER USER is part of the user management rule.
        | {self.serverVersion >= 50713}? INSTANCE_SYMBOL ROTATE_SYMBOL textOrIdentifier MASTER_SYMBOL KEY_SYMBOL
    )
;
...
alterDatabase:
    DATABASE_SYMBOL schemaRef (
        ...
        | {self.serverVersion < 80000}? UPGRADE_SYMBOL DATA_SYMBOL DIRECTORY_SYMBOL NAME_SYMBOL
    )
;
...
alterTable:
    onlineOption? ({self.serverVersion < 50700}? IGNORE_SYMBOL)? TABLE_SYMBOL tableRef alterTableActions?
;
...
standaloneAlterCommands:
    ...
    | alterPartition
    | {self.serverVersion >= 80014}? (SECONDARY_LOAD_SYMBOL | SECONDARY_UNLOAD_SYMBOL)
;
...
alterPartition:
    ...
    | {self.serverVersion >= 50704}? DISCARD_SYMBOL PARTITION_SYMBOL allOrPartitionNameList TABLESPACE_SYMBOL
    | {self.serverVersion >= 50704}? IMPORT_SYMBOL PARTITION_SYMBOL allOrPartitionNameList TABLESPACE_SYMBOL
;
...
alterListItem:
    ...
    | DROP_SYMBOL (
        ...
        | FOREIGN_SYMBOL KEY_SYMBOL (
            ...
            {self.serverVersion >= 50700}? columnInternalRef
            | {self.serverVersion < 50700}? columnInternalRef?
        )
        | PRIMARY_SYMBOL KEY_SYMBOL
        | keyOrIndex indexRef
        | {self.serverVersion >= 80017}? CHECK_SYMBOL identifier
        | {self.serverVersion >= 80019}? CONSTRAINT_SYMBOL identifier
    )
    | DISABLE_SYMBOL KEYS_SYMBOL
    | ENABLE_SYMBOL KEYS_SYMBOL
    | ALTER_SYMBOL COLUMN_SYMBOL? columnInternalRef (
        SET_SYMBOL DEFAULT_SYMBOL (
            {self.serverVersion >= 80014}? exprWithParentheses
            | signedLiteral
        )
        | DROP_SYMBOL DEFAULT_SYMBOL
    )
    | {self.serverVersion >= 80000}? ALTER_SYMBOL INDEX_SYMBOL indexRef visibility
    | {self.serverVersion >= 80017}? ALTER_SYMBOL CHECK_SYMBOL identifier constraintEnforcement
    | {self.serverVersion >= 80019}? ALTER_SYMBOL CONSTRAINT_SYMBOL identifier constraintEnforcement
    | {self.serverVersion >= 80000}? RENAME_SYMBOL COLUMN_SYMBOL columnInternalRef TO_SYMBOL identifier
    | RENAME_SYMBOL (TO_SYMBOL | AS_SYMBOL)? tableName
    | {self.serverVersion >= 50700}? RENAME_SYMBOL keyOrIndex indexRef TO_SYMBOL indexName
    | CONVERT_SYMBOL TO_SYMBOL charset (
        {self.serverVersion >= 80014}? DEFAULT_SYMBOL
        | charsetName
    ) collate?
    | FORCE_SYMBOL
    | ORDER_SYMBOL BY_SYMBOL alterOrderList
    | {self.serverVersion >= 50708 and self.serverVersion < 80000}? UPGRADE_SYMBOL PARTITIONING_SYMBOL
;
...
withValidation:
    {self.serverVersion >= 50706}? (WITH_SYMBOL | WITHOUT_SYMBOL) VALIDATION_SYMBOL
;
...
alterTablespace:
    TABLESPACE_SYMBOL tablespaceRef (
        ...
        | RENAME_SYMBOL TO_SYMBOL identifier
        | {self.serverVersion >= 80014}? alterTablespaceOptions
    )
;
...
createStatement:
    CREATE_SYMBOL (
        ...
        | createTablespace
        | createEvent
        | {self.serverVersion >= 80000}? createRole
        | {self.serverVersion >= 80011}? createSpatialReference
        | {self.serverVersion >= 80014}? createUndoTablespace
    )
;
...
createDatabaseOption:
    defaultCharset
    | defaultCollation
    | {self.serverVersion >= 80016}? defaultEncryption
;
...
createIndex:
    onlineOption? (
        UNIQUE_SYMBOL? type = INDEX_SYMBOL (
            {self.serverVersion >= 80014}? indexName indexTypeClause?
            | indexNameAndType?
...
tsDataFileName:
    {self.serverVersion >= 80014}? (ADD_SYMBOL tsDataFile)?
...
tablespaceOption:
    ...
    | tsOptionWait
    | tsOptionComment
    | {self.serverVersion >= 50707}? tsOptionFileblockSize
    | {self.serverVersion >= 80014}? tsOptionEncryption
;
...
triggerFollowsPrecedesClause:
    {self.serverVersion >= 50700}? ordering = (FOLLOWS_SYMBOL | PRECEDES_SYMBOL) textOrIdentifier // not a trigger reference!
;
...
dropStatement:
    DROP_SYMBOL (
        ...
        | dropTrigger
        | dropView
        | {self.serverVersion >= 80000}? dropRole
        | {self.serverVersion >= 80011}? dropSpatialReference
        | {self.serverVersion >= 80014}? dropUndoTablespace
    )
;
...
deleteStatement:
    ({self.serverVersion >= 80000}? withClause)? DELETE_SYMBOL deleteStatementOption* (
        FROM_SYMBOL (
            tableAliasRefList USING_SYMBOL tableReferenceList whereClause?       // Multi table variant 1.
            | tableRef ({self.serverVersion >= 80017}? tableAlias)? partitionDelete?
...
partitionDelete:
    {self.serverVersion >= 50602}? PARTITION_SYMBOL OPEN_PAR_SYMBOL identifierList CLOSE_PAR_SYMBOL
;
...
doStatement:
    DO_SYMBOL (
        {self.serverVersion < 50709}? exprList
        | {self.serverVersion >= 50709}? selectItemList
    )
;
...
insertStatement:
    INSERT_SYMBOL insertLockOption? IGNORE_SYMBOL? INTO_SYMBOL? tableRef usePartition? (
        insertFromConstructor ({ self.serverVersion >= 80018}? valuesReference)?
        | SET_SYMBOL updateList ({ self.serverVersion >= 80018}? valuesReference)?
...
queryExpression:
    ({self.serverVersion >= 80000}? withClause)? (
        queryExpressionBody orderClause? limitClause?
        | queryExpressionParens orderClause? limitClause?
    ) ({self.serverVersion < 80000}? procedureAnalyseClause)?
;

/* Make having parens around the query require at least one union. */
queryExpressionBody:
    (
        queryPrimary
...
queryPrimary:
    querySpecification
    | {self.serverVersion >= 80019}? tableValueConstructor
    | {self.serverVersion >= 80019}? explicitTable
;
...
olapOption:
    ...
    | {self.serverVersion < 80000}? WITH_SYMBOL CUBE_SYMBOL
;
...
selectOption:
    ...
    | {self.serverVersion < 80000}? SQL_CACHE_SYMBOL
    | {self.serverVersion >= 50704 and self.serverVersion < 50708}? MAX_STATEMENT_TIME_SYMBOL EQUAL_OPERATOR real_ulong_number
;
...
lockingClause:
    FOR_SYMBOL lockStrengh ({self.serverVersion >= 80000}? OF_SYMBOL tableAliasRefList)? (
        {self.serverVersion >= 80000}? lockedRowAction
    )?
...
lockStrengh:
    UPDATE_SYMBOL
    | {self.serverVersion >= 80000}? SHARE_SYMBOL
;
...
tableReference: ( // Note: we have also a tableRef rule for identifiers that reference a table anywhere.
        tableFactor
        | OPEN_CURLY_SYMBOL ({self.serverVersion < 80017}? identifier | OJ_SYMBOL) escapedTableReference CLOSE_CURLY_SYMBOL // ODBC syntax
    ) joinedTable*
;
...
tableFactor:
    ...
    | {self.serverVersion >= 80004}? tableFunction
;
...
derivedTable:
    subquery tableAlias? ({self.serverVersion >= 80000}? columnInternalRefList)?
    | {self.serverVersion >= 80014}? LATERAL_SYMBOL subquery tableAlias? columnInternalRefList?
;
...
jtColumn:
    ...
    | identifier dataType ({self.serverVersion >= 80014}? collate)? EXISTS_SYMBOL? PATH_SYMBOL textStringLiteral onEmptyOrError?
...
tableAlias:
    (AS_SYMBOL | {self.serverVersion < 80017}? EQUAL_OPERATOR)? identifier
;
...
updateStatement:
    ({self.serverVersion >= 80000}? withClause)? UPDATE_SYMBOL LOW_PRIORITY_SYMBOL? IGNORE_SYMBOL? tableReferenceList SET_SYMBOL
...
transactionCharacteristic:
    ...
    | {self.serverVersion >= 50605}? READ_SYMBOL (WRITE_SYMBOL | ONLY_SYMBOL)
;
...
lockStatement:
    ...
    | {self.serverVersion >= 80000}? LOCK_SYMBOL INSTANCE_SYMBOL FOR_SYMBOL BACKUP_SYMBOL
    | UNLOCK_SYMBOL (
        TABLES_SYMBOL
        | TABLE_SYMBOL
        | {self.serverVersion >= 80000}? INSTANCE_SYMBOL
    )
;
...
xaConvert:
    {self.serverVersion >= 50704}? (CONVERT_SYMBOL XID_SYMBOL)?
    | /* empty */
;
...
replicationStatement:
    ...
    | {self.serverVersion > 80000}? RESET_SYMBOL PERSIST_SYMBOL (ifExists identifier)?
    | slave
    | {self.serverVersion >= 50700}? changeReplication
    | replicationLoad
    | {self.serverVersion > 50706}? groupReplication
;

resetOption:
    option = MASTER_SYMBOL masterResetOptions?
    | {self.serverVersion < 80000}? option = QUERY_SYMBOL CACHE_SYMBOL
    | option = SLAVE_SYMBOL ALL_SYMBOL? channel?
;

masterResetOptions:
    {self.serverVersion >= 80000}? TO_SYMBOL (
        {self.serverVersion < 80017}? real_ulong_number
        | {self.serverVersion >= 80017}? real_ulonglong_number
    )
;
...
changeReplication:
    ...
    )* ({self.serverVersion >= 80000}? channel)?
;
...
slaveUntilOptions:
    (
        masterFileDef
        | {self.serverVersion >= 50606}? (
            SQL_BEFORE_GTIDS_SYMBOL
            | SQL_AFTER_GTIDS_SYMBOL
        ) EQUAL_OPERATOR textString
        | {self.serverVersion >= 50606}? SQL_AFTER_MTS_GAPS_SYMBOL
    ) (COMMA_SYMBOL masterFileDef)*
;

slaveConnectionOptions:
    {self.serverVersion >= 50604}? (USER_SYMBOL EQUAL_OPERATOR textString)? (
...
cloneStatement:
    CLONE_SYMBOL (
        ...
        | {self.serverVersion >= 80014}? INSTANCE_SYMBOL FROM_SYMBOL user COLON_SYMBOL ulong_number IDENTIFIED_SYMBOL BY_SYMBOL
...
accountManagementStatement:
    {self.serverVersion >= 50606}? alterUser
    | createUser
    | dropUser
    | grant
    | renameUser
    | revoke
    | {self.serverVersion >= 80000}? setRole
;

alterUser:
    ALTER_SYMBOL USER_SYMBOL ({self.serverVersion >= 50706}? ifExists)? alterUserTail
;

alterUserTail:
    ({self.serverVersion < 80014}? createUserList | {self.serverVersion >= 80014}? alterUserList) createUserTail
    | {self.serverVersion >= 50706}? user IDENTIFIED_SYMBOL BY_SYMBOL textString (
        {self.serverVersion >= 80014}? replacePassword
    )? ({self.serverVersion >= 80014}? retainCurrentPassword)?
    | {self.serverVersion >= 80014}? user discardOldPassword
    | {self.serverVersion >= 80000}? user DEFAULT_SYMBOL ROLE_SYMBOL (
        ALL_SYMBOL
        | NONE_SYMBOL
        | roleList
    )
    | {self.serverVersion >= 80018}? user IDENTIFIED_SYMBOL (WITH_SYMBOL textOrIdentifier)? BY_SYMBOL RANDOM_SYMBOL
...
createUser:
    CREATE_SYMBOL USER_SYMBOL ({self.serverVersion >= 50706}? ifNotExists | /* empty */) createUserList defaultRoleClause
        createUserTail
;

createUserTail:
    {self.serverVersion >= 50706}? requireClause? connectOptions? accountLockPasswordExpireOptions*
    | /* empty */
;

defaultRoleClause:
    {self.serverVersion >= 80000}? (DEFAULT_SYMBOL ROLE_SYMBOL roleList)?
    | /* empty */
;
...
accountLockPasswordExpireOptions:
    ...
    | PASSWORD_SYMBOL (
        ...
        | {self.serverVersion >= 80014}? REQUIRE_SYMBOL CURRENT_SYMBOL (
            DEFAULT_SYMBOL
            | OPTIONAL_SYMBOL
        )?
    )
...
dropUser:
    DROP_SYMBOL USER_SYMBOL ({self.serverVersion >= 50706}? ifExists)? userList
;

grant:
    GRANT_SYMBOL (
        {self.serverVersion >= 80000}? roleOrPrivilegesList TO_SYMBOL userList (
...
grantTargetList:
    {self.serverVersion < 80011}? createUserList
    | {self.serverVersion >= 80011}? userList
;

grantOptions:
    {self.serverVersion < 80011}? WITH_SYMBOL grantOption+
    | {self.serverVersion >= 80011}? WITH_SYMBOL GRANT_SYMBOL OPTION_SYMBOL
;
...
versionedRequireClause:
    {self.serverVersion < 80011}? requireClause
;
...
revoke:
    REVOKE_SYMBOL (
        {self.serverVersion >= 80000}? roleOrPrivilegesList FROM_SYMBOL userList
        | roleOrPrivilegesList onTypeTo FROM_SYMBOL userList
        | ALL_SYMBOL PRIVILEGES_SYMBOL? (
            {self.serverVersion >= 80000}? ON_SYMBOL aclType? grantIdentifier
...
roleOrPrivilege:
    ...
    | {self.serverVersion > 80000}? (CREATE_SYMBOL | DROP_SYMBOL) ROLE_SYMBOL
;
...
grantIdentifier:
    ...
    | tableRef
    | {self.serverVersion >= 80017}? schemaRef DOT_SYMBOL tableRef
;
...
tableAdministrationStatement:
    type = ANALYZE_SYMBOL noWriteToBinLog? TABLE_SYMBOL tableRefList (
        {self.serverVersion >= 80000}? histogram
    )?
...
startOptionValueList:
    ...
    | PASSWORD_SYMBOL (FOR_SYMBOL user)? equal (
        ...
        | textString replacePassword? retainCurrentPassword?
        | {self.serverVersion < 50706}? OLD_PASSWORD_SYMBOL OPEN_PAR_SYMBOL textString CLOSE_PAR_SYMBOL
        | {self.serverVersion < 80014}? PASSWORD_SYMBOL OPEN_PAR_SYMBOL textString CLOSE_PAR_SYMBOL
    )
    | {self.serverVersion >= 80018}? PASSWORD_SYMBOL (FOR_SYMBOL user)? TO_SYMBOL RANDOM_SYMBOL replacePassword? retainCurrentPassword?
;
...
optionValueNoOptionType:
    ...
    | NAMES_SYMBOL (
        equal expr
        | charsetName collate?
        | {self.serverVersion >= 80011}? DEFAULT_SYMBOL
    )
;
...
setExprOrDefault:
    ...
    | {self.serverVersion >= 80000}? (ROW_SYMBOL | SYSTEM_SYMBOL)
;
...
showStatement:
    SHOW_SYMBOL (
        {self.serverVersion < 50700}? value = AUTHORS_SYMBOL
        ...
        | ({self.serverVersion >= 80000}? EXTENDED_SYMBOL)? value = (
            INDEX_SYMBOL
            | INDEXES_SYMBOL
            | KEYS_SYMBOL
        ...
        | {self.serverVersion < 50700}? value = CONTRIBUTORS_SYMBOL
        ...
        | value = CREATE_SYMBOL (
            ...
            | {self.serverVersion >= 50704}? object = USER_SYMBOL user
        )
...
showCommandType:
    FULL_SYMBOL
    | {self.serverVersion >= 80000}? EXTENDED_SYMBOL FULL_SYMBOL?
;

nonBlocking:
    {self.serverVersion >= 50700 and self.serverVersion < 50706}? NONBLOCKING_SYMBOL?
    | /* empty */
;
...
otherAdministrativeStatement:
    ...
    | {self.serverVersion >= 50709}? type = SHUTDOWN_SYMBOL
;
...
flushOption:
    ...
    | {self.serverVersion < 80000}? option = QUERY_SYMBOL CACHE_SYMBOL
    | {self.serverVersion >= 50706}? option = OPTIMIZER_COSTS_SYMBOL
;
...
flushTablesOptions:
    {self.serverVersion >= 50606}? FOR_SYMBOL EXPORT_SYMBOL
    | WITH_SYMBOL READ_SYMBOL LOCK_SYMBOL
;
...
utilityStatement:
    ...
    | helpCommand
    | useCommand
    | {self.serverVersion >= 80011}? restartServer
;
...
explainStatement:
    (EXPLAIN_SYMBOL | DESCRIBE_SYMBOL | DESC_SYMBOL) (
        {self.serverVersion < 80000}? EXTENDED_SYMBOL
        | {self.serverVersion < 80000}? PARTITIONS_SYMBOL
        | {self.serverVersion >= 50605}? FORMAT_SYMBOL EQUAL_OPERATOR textOrIdentifier
        | {self.serverVersion >= 80018}? ANALYZE_SYMBOL
        | {self.serverVersion >= 80019}? ANALYZE_SYMBOL FORMAT_SYMBOL EQUAL_OPERATOR textOrIdentifier
...
explainableStatement:
    selectStatement
    | {self.serverVersion >= 50603}? (
        deleteStatement
        | insertStatement
        | replaceStatement
        | updateStatement
    )
    | {self.serverVersion >= 50700}? FOR_SYMBOL CONNECTION_SYMBOL real_ulong_number
;
...
predicate:
    bitExpr (
        ...
        | {self.serverVersion >= 80017}? MEMBER_SYMBOL OF_SYMBOL? simpleExprWithParentheses
...
simpleExpr:
    ...
    | {self.serverVersion >= 80000}? groupingOperation                                                        # simpleExprGroupingOperation
    | {self.serverVersion >= 80000}? windowFunctionCall                                                       # simpleExprWindowingFunction
...
arrayCast:
    {self.serverVersion >= 80017}? ARRAY_SYMBOL
;

jsonOperator:
    {self.serverVersion >= 50708}? JSON_SEPARATOR_SYMBOL textStringLiteral
    | {self.serverVersion >= 50713}? JSON_UNQUOTED_SEPARATOR_SYMBOL textStringLiteral
;

sumExpr:
    name = AVG_SYMBOL OPEN_PAR_SYMBOL DISTINCT_SYMBOL? inSumExpr CLOSE_PAR_SYMBOL (
        {self.serverVersion >= 80000}? windowingClause
    )?
    | name = (BIT_AND_SYMBOL | BIT_OR_SYMBOL | BIT_XOR_SYMBOL) OPEN_PAR_SYMBOL inSumExpr CLOSE_PAR_SYMBOL (
        {self.serverVersion >= 80000}? windowingClause
    )?
    | {self.serverVersion >= 80000}? jsonFunction
    ...
    ) CLOSE_PAR_SYMBOL ({self.serverVersion >= 80000}? windowingClause)?
...
runtimeFunctionCall:
    ...
    | {self.serverVersion < 50607}? name = OLD_PASSWORD_SYMBOL OPEN_PAR_SYMBOL textLiteral CLOSE_PAR_SYMBOL
    | {self.serverVersion < 80011}? name = PASSWORD_SYMBOL exprWithParentheses
    ...
    | name = WEIGHT_STRING_SYMBOL OPEN_PAR_SYMBOL expr (
        (AS_SYMBOL CHAR_SYMBOL wsNumCodepoints)? (
            {self.serverVersion < 80000}? weightStringLevels
        )?
...
geometryFunction:
    {self.serverVersion < 50706}? name = CONTAINS_SYMBOL OPEN_PAR_SYMBOL expr COMMA_SYMBOL expr CLOSE_PAR_SYMBOL
...
fractionalPrecision:
    {self.serverVersion >= 50604}? INT_NUMBER
;
...
internalVariableName: (
        ...
        {self.serverVersion < 80017}? identifier dotIdentifier?
        | {self.serverVersion >= 80017}? lValueIdentifier dotIdentifier?
    )
...
castType:
    ...
    | {self.serverVersion >= 50708}? JSON_SYMBOL
    | {self.serverVersion >= 80017}? realType
    | {self.serverVersion >= 80017}? FLOAT_SYMBOL standardFloatOptions?
;
...
channel:
    {self.serverVersion >= 50706}? FOR_SYMBOL CHANNEL_SYMBOL textStringNoLinebreak
;
...
getDiagnostics:
    GET_SYMBOL (CURRENT_SYMBOL | {self.serverVersion >= 50700}? STACKED_SYMBOL)? DIAGNOSTICS_SYMBOL (
...
checkOrReferences:
    {self.serverVersion < 80016}? checkConstraint
    | references
;
...
tableConstraintDef:
    ...
    | constraintName? (
        ...
        | checkConstraint ({self.serverVersion >= 80017}? constraintEnforcement)?
    )
;
...
fieldDefinition:
    dataType (
        columnAttribute*
        | {self.serverVersion >= 50707}? collate? (GENERATED_SYMBOL ALWAYS_SYMBOL)? AS_SYMBOL exprWithParentheses (
            VIRTUAL_SYMBOL
            | STORED_SYMBOL
        )? (
            {self.serverVersion < 80000}? gcolAttribute*
            | {self.serverVersion >= 80000}? columnAttribute* // Beginning with 8.0 the full attribute set is supported.
        )
    )
;
...
columnAttribute:
    ...
    | {self.serverVersion >= 80014}? NOT_SYMBOL SECONDARY_SYMBOL
    | value = DEFAULT_SYMBOL (
        signedLiteral
        | NOW_SYMBOL timeFunctionParameters?
        | {self.serverVersion >= 80013}? exprWithParentheses
    )
    ...
    | {self.serverVersion >= 80000}? value = SRID_SYMBOL real_ulonglong_number
    | {self.serverVersion >= 80017}? constraintName? checkConstraint
    | {self.serverVersion >= 80017}? constraintEnforcement
;
...
keyListVariants:
    {self.serverVersion >= 80013}? keyListWithExpression
    | {self.serverVersion < 80013}? keyList
;
...
commonIndexOption:
    ...
    | COMMENT_SYMBOL textLiteral
    | {self.serverVersion >= 80000}? visibility
;
...
dataType: // type in sql_yacc.yy
    ...
    | type = SERIAL_SYMBOL
    | {self.serverVersion >= 50708}? type = JSON_SYMBOL
    | type = (
        GEOMETRY_SYMBOL
...
collationName:
    textOrIdentifier
    | {self.serverVersion < 80011}? DEFAULT_SYMBOL
    | {self.serverVersion >= 80018}? BINARY_SYMBOL
;
...
createTableOption: // In the order as they appear in the server grammar.
    ...
    | {self.serverVersion >= 80014}? option = SECONDARY_ENGINE_SYMBOL equal? (
        NULL_SYMBOL
        | textOrIdentifier
    )
    ...
    | {self.serverVersion >= 50708}? option = COMPRESSION_SYMBOL EQUAL_OPERATOR? textString
    | {self.serverVersion >= 50711}? option = ENCRYPTION_SYMBOL EQUAL_OPERATOR? textString
    ...
    | option = TABLESPACE_SYMBOL (
        {self.serverVersion >= 50707}? EQUAL_OPERATOR?
        | /* empty */
    ) identifier
...
partitionKeyAlgorithm: // Actually only 1 and 2 are allowed. Needs a semantic check.
    {self.serverVersion >= 50700}? ALGORITHM_SYMBOL EQUAL_OPERATOR real_ulong_number
;
...
createUserEntry: // create_user in sql_yacc.yy
    user (
        IDENTIFIED_SYMBOL (
            BY_SYMBOL ({self.serverVersion < 80011}? PASSWORD_SYMBOL)? textString
            | WITH_SYMBOL textOrIdentifier (
                AS_SYMBOL textStringHash
                | {self.serverVersion >= 50706}? BY_SYMBOL textString
            )?
            | {self.serverVersion >= 80018}? (WITH_SYMBOL textOrIdentifier)? BY_SYMBOL RANDOM_SYMBOL PASSWORD_SYMBOL
        )
    )?
;
...
usePartition:
    {self.serverVersion >= 50602}? PARTITION_SYMBOL identifierListWithParentheses
;
...
columnName:
    ...
    {self.serverVersion >= 80000}? identifier
    | {self.serverVersion < 80000}? fieldIdentifier
;
...
pureIdentifier:
    ...
    | {self.isSqlModeActive(AnsiQuotes)}? DOUBLE_QUOTED_TEXT
;
...
simpleIdentifier: // simple_ident + simple_ident_q
    ...
    | {self.serverVersion < 80000}? dotIdentifier dotIdentifier
;
...
real_ulonglong_number:
    INT_NUMBER
    | {self.serverVersion >= 80017}? HEX_NUMBER
    | ULONGLONG_NUMBER
    | LONG_NUMBER
;
...
textStringLiteral:
    ...
    | {not self.isSqlModeActive(AnsiQuotes)}? value = DOUBLE_QUOTED_TEXT
;
...
identifierKeyword:
    {self.serverVersion < 80017}? (
        labelKeyword
        | roleOrIdentifierKeyword
        | EXECUTE_SYMBOL
        | {self.serverVersion >= 50709}? SHUTDOWN_SYMBOL // Previously allowed as SP label as well.
        | {self.serverVersion >= 80011}? RESTART_SYMBOL
    )
    | (
...
roleKeyword:
    {self.serverVersion < 80017}? (roleOrLabelKeyword | roleOrIdentifierKeyword)
    | (
        identifierKeywordsUnambiguous
        | identifierKeywordsAmbiguous2Labels
        | identifierKeywordsAmbiguous4SystemVariables
...
roleOrIdentifierKeyword:
    ...
    | {self.serverVersion >= 50709}? SHUTDOWN_SYMBOL
    | {self.serverVersion >= 80000}? IMPORT_SYMBOL
;
...
roleOrLabelKeyword:
    ...
    | {self.serverVersion >= 80014}? ADMIN_SYMBOL
;
