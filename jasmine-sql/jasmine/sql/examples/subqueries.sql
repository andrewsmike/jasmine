...
SELECT ( SELECT SUM ( cost )
  FROM a_sub
 WHERE b.a_id = a_sub.id ) 1
  FROM blah.my_table a JOIN bleh.extra_table b ON a.id = db.b.a_id LEFT JOIN ( SELECT a b
  FROM c JOIN d USING ( blah ) ) l ON l.b_id = b.id
 WHERE a.a_blah = 4.4 OR a.b_blah = 3 AND b.eai IN ( SELECT eai
  FROM good_eai )
 GROUP BY a b 1
HAVING c ORDER BY a LIMIT 1;
*/
